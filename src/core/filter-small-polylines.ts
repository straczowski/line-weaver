...
export const filterSmallPolylines = ({
  polylines,
  minLength,
}: FilterSmallPolylinesInput): FilterSmallPolylinesOutput => {
  if (minLength <= 0) {
    return {
      polylines,
      removedCount: 0,
  ...
  const filteredPolylines = polylines.filter(
    (polyline) => calculatePolylineLength({ polyline }) >= minLength
  )
