...
import type { BrightnessGrid } from "./types"
...
describe("generateAdvancedHatching", () => {
  ...
  it("should generate no lines for light cells", () => {
    const grid = createBrightnessGrid([[255, 255], [255, 255]])
  ...
  it("should generate lines for dark cells", () => {
    const grid = createBrightnessGrid([[0, 0], [0, 0]])
  ...
  it("should generate more lines for darker cells", () => {
    ...
    assert.ok(
      darkResult.length >= lightResult.length,
      `Dark cells should have more lines: light=${lightResult.length}, dark=${darkResult.length}`
    )
  })
  ...
  it("should respect maxDensity setting", () => {
    ...
    assert.ok(
      highDensity.length > lowDensity.length,
      `Higher density should produce more lines: low=${lowDensity.length}, high=${highDensity.length}`
    )
  })
  ...
  it("should add cross-hatch lines when enabled", () => {
    ...
    assert.ok(
      withCrossHatch.length > withoutCrossHatch.length,
      `Cross-hatch should add more lines: with=${withCrossHatch.length}, without=${withoutCrossHatch.length}`
    )
  })
