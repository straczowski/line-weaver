...
export const generateSvgPaths = (params: GenerateSvgPathsInput): string[] => {
  ...
  return polylines.filter(isNonEmptyPolyline).map((polyline) => createPathElement(polyline, strokeConfig))
}
...
const createStrokeAttributes = (config: SvgStrokeConfig): string => `stroke="${config.color}" stroke-width="${config.width}" stroke-linecap="${config.linecap}" stroke-linejoin="${config.linejoin}"`
