...
const normalizeKernel = (kernel: number[][], sum: number): number[][] => kernel.map((row) => row.map((value) => value / sum))

const applyConvolution = (params: { grayscaleData: GrayscaleData; kernel: number[][] }): Uint8Array => {
...
const computeConvolvedValue = (params: { x: number; y: number; width: number; height: number; pixels: Uint8Array; kernel: number[][]; radius: number }): number => {
