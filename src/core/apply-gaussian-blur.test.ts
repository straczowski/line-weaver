...
describe("applyGaussianBlur", () => {
  ...
  it("should smooth out pixel variations", () => {
    ...
    assert.ok(
      blurredVariance < originalVariance,
      `Variance should decrease: original=${originalVariance}, blurred=${blurredVariance}`
    )
  })
  ...
  it("should blur more with larger radius", () => {
    ...
    assert.ok(
      variance2 < variance1,
      `Larger radius should blur more: r1 variance=${variance1}, r3 variance=${variance2}`
    )
  })
