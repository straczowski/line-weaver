import type { BrightnessGrid, LinePattern, Point, Polyline } from "./types"
...
const generateCross = (x: number, y: number, size: number): Polyline[] => [
  generateDiagonalRight(x, y, size),
  generateDiagonalLeft(x, y, size),
]

const generateHatch = (x: number, y: number, size: number): Polyline[] => [
  generateDiagonalRight(x, y, size),
  generateDiagonalLeft(x, y, size),
  [createPoint(x, y + size / 2), createPoint(x + size, y + size / 2)],
  [createPoint(x + size / 2, y), createPoint(x + size / 2, y + size)],
]
